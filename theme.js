  ...
  allElements.forEach(el => {
    if (isLight) {
      // Light mode → force dark text where it was white
      const current = window.getComputedStyle(el).color;
      const rgb = current.match(/\d+/g);
      if (rgb && parseInt(rgb[0]) > 220 && parseInt(rgb[1]) > 220 && parseInt(rgb[2]) > 220) {
        el.style.color = '#0f172a';
      }
    } else {
      // Dark mode → REMOVE inline color completely
      if (el.style.color) {
        el.style.color = '';           // ← this is the critical line missing before
      }
    }
  });
}
